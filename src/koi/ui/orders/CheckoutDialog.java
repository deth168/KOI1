...
public class CheckoutDialog extends JDialog {
	...
	private DrinkMenuDialog drinkMenuDialog;
	...
	public CheckoutDialog(DrinkMenuDialog drinkMenuDialog, BillingApp frame, DrinkDAO drinkDAO, Staff staff) {
		
		this.drinkMenuDialog = drinkMenuDialog;
		this.frame = frame;
		...
		addWindowListener(new WindowAdapter() {
			  ...
			  { 
			    ...
			    {
			    	drinkDAO.vacateQuantityColumn();
			    	System.exit(0);          
			    }
			  }
			});
		
		setTitle("KOI The - Checkout");
		...
		JLabel lblSelectTheProduct = new JLabel("Select the drink and click Delete button to delete from cart");
		...
		btnBack.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				dispose();
				drinkMenuDialog.refreshDrinkItemView();
				drinkMenuDialog.setVisible(true);
			}
		});
	...
	public void setNetAmountLabel(){
		try {
			netAmount = drinkDAO.getNetAmount();
	...
	private void setCartTableModel(){
		try {
			tableModel = new CartTableModel(drinkDAO.getCartItems());
	...
	private void deleteItem(){
		...
		DrinkItem temp = (DrinkItem) cartTable.getValueAt(row, CartTableModel.OBJECT_COL);
		try {
			
			drinkDAO.setQuantityToNull(temp);
			setCartTableModel();
			setNetAmountLabel();
	...
	private void placeOrder(){
		...
		try {
			int orderId = (int) drinkDAO.addOrder(staff, netAmount);	//insert the order details in order menu.
			drinkDAO.vacateQuantityColumn();
