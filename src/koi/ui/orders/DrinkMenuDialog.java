...
public class DrinkMenuDialog extends JDialog {
	...
	private DrinkTableModel tableModel;
	...
	public DrinkMenuDialog(BillingApp frame, DrinkDAO drinkDAO, Staff staff) {
		...
		btnAddToCart.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent ae) {
				...
				try{
					...
					if(row<0){
						JOptionPane.showMessageDialog(DrinkMenuDialog.this, "You must select a drink item",
					...
					DrinkItem tempDrinkItem = (DrinkItem)drinktable.getValueAt(row, DrinkTableModel.OBJECT_COL);					
					...
					*/
					...
					drinkDAO.updateQuantity(tempDrinkItem, quantity);
					
					refreshDrinkItemView();
					JOptionPane.showMessageDialog(DrinkMenuDialog.this, "Drink item added to cart.",
							"Cart", JOptionPane.INFORMATION_MESSAGE);				
				}
				catch(Exception e){
					JOptionPane.showMessageDialog(DrinkMenuDialog.this, "Error adding drink to cart: "
		...
		try{
			tableModel = new DrinkTableModel(drinkDAO.getAllDrinkItems());
			drinktable.setModel(tableModel);
			
			//tabelscrollPane.add(foodtable.getTableHeader(), BorderLayout.NORTH);
			tabelscrollPane.setViewportView(drinktable);
			alignTable();
		...
		btnProceedToCheckout.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					boolean check = drinkDAO.isEmptyQuantityColumn();
					if(!check){
						JOptionPane.showMessageDialog(DrinkMenuDialog.this, "Your cart is empty",
				...
				CheckoutDialog dialog = new CheckoutDialog(DrinkMenuDialog.this, frame, drinkDAO, staff);
	...
	private void alignTable(){
		...
		drinktable.setDefaultRenderer(String.class, centerRenderer);

		//Centering particular columns
		drinktable.getColumnModel().getColumn(0).setCellRenderer( centerRenderer );
		drinktable.getColumnModel().getColumn(3).setCellRenderer( centerRenderer );
		drinktable.getColumnModel().getColumn(4).setCellRenderer( centerRenderer );
		// can use this also
		//foodtable.setDefaultRenderer(foodtable.getColumnClass(0), centerRenderer);
		
	}
	
	public void refreshDrinkItemView(){		
		try {
			tableModel = new DrinkTableModel(drinkDAO.getAllDrinkItems());
			drinktable.setModel(tableModel);
			alignTable();
